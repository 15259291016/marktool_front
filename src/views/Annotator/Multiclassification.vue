    ...
    <div class="left">
      ...
      <div class="textarea" @mousewheel="selectNextPage">
        ...
        <div class="labeled" :v-model="allIntentions[$store.state.dataInfo.currentLocation].label">
          <span
            @contextmenu.prevent="deleteLabel(allIntentions[$store.state.dataInfo.currentLocation].label)"
            style="padding-left:5px; padding-right:5px;"
          >{{ allIntentions[$store.state.dataInfo.currentLocation].label }}</span>
          <span
            style="position: absolute; right: 30px;"
          >{{ $store.state.dataInfo.currentLocation + 1 }}</span>
        </div>
      </div>
      <div style="margin-left: 2%;background-color: red;">如果标签多点一次，出现重复的，请删除标签重新选取标签</div>
      <!-----------------------          标注按钮区          ----------------------->
...
export default {
  ...
  created: function () {
    if(this.$store.state.dataInfo.filename){
      sessionStorage.setItem('filename', this.$store.state.dataInfo.filename)
    }
    ...
    this.$axios({
      ...
      params: {
        filename: this.$store.state.dataInfo.filename ? this.$store.state.dataInfo.filename : sessionStorage.getItem('filename')
      }
    }).then(res => {
      // debugger
  ...
  methods: {
    readConfig () {
      var department = this.$store.state.dataInfo.filename.split('_')[0] ? this.$store.state.dataInfo.filename.split('_')[0] : sessionStorage.getItem('filename').split('_')[0]
      this.$axios({
        method: 'GET',
        ...
        .then(res => {
          if (res['data']['code'] === 200) {
            this.classifications = res['data']['data']['config']['multiclassification']
          }
        }).catch(error =>
    ...
    selectNextPage (e) {
      console.log(this.allIntentions)
      console.log(this.$store.state.dataInfo.currentLocation)
      // debugger
      let label_list = (this.allIntentions[this.$store.state.dataInfo.currentLocation].label)
      let uuid = this.allIntentions[this.$store.state.dataInfo.currentLocation].uuid
      let filename = this.allIntentions[this.$store.state.dataInfo.currentLocation].file
      let label_ = label_list
      let ner = ''
      let tuple = ''
      this.upload(uuid, filename, label_, ner, tuple)
      if (e['deltaY'] > 0) {
        if (this.$store.state.dataInfo.currentLocation === (this.allIntentions.length - 1)) {
          // this.$message({
          //   message: "已经是最后一个了",
          //   type: "error",
          //   duration: 2000
          ...
          // this.upload("", filename, label_, ner, tuple);
    ...
    // 删除
    deleteLabel (val) {
      this.allIntentions[this.$store.state.dataInfo.currentLocation].label.splice(this.allIntentions[this.$store.state.dataInfo.currentLocation].label.indexOf(val), 1)
      if (this.allIntentions[this.$store.state.dataInfo.currentLocation].label.length === 0) {
        this.allIntentions[this.$store.state.dataInfo.currentLocation]['marked'] = 'sentence-no-mark'
        this.$store.state.dataInfo.dataMarkedInfo[this.$store.state.dataInfo.currentLocation].marked = 'sentence-no-mark'
      ...
      let uuid = this.allIntentions[this.$store.state.dataInfo.currentLocation].uuid
      let filename = this.allIntentions[this.$store.state.dataInfo.currentLocation].file
      let label = this.allIntentions[this.$store.state.dataInfo.currentLocation].label.join('#')
      let ner = ''
      let tuple = ''
    ...
    earlyUpload (category) {
      if (this.allIntentions[this.$store.state.dataInfo.currentLocation].label !== '') {
    ...
    uploadLabel (category) {
      // 标注意图标签
      // debugger
      var label_list = (this.allIntentions[this.$store.state.dataInfo.currentLocation].label + ',' + category)
      let label = this.earlyUpload(label_list)
      let uuid = this.allIntentions[this.$store.state.dataInfo.currentLocation].uuid
      let filename = this.allIntentions[this.$store.state.dataInfo.currentLocation].file
      let label_ = label_list
      let ner = ''
      let tuple = ''
      // 统计标注信息
      if (this.allIntentions[this.$store.state.dataInfo.currentLocation].label.length > 0 && category === label_) {
        this.$store.state.dataInfo.marked += 1
        this.$store.state.dataInfo.unmark -= 1
        // 自动翻页
        // this.$store.state.dataInfo.dataMarkedInfo[this.$store.state.dataInfo.currentLocation].marked = "sentence-mark";
        // this.$store.state.dataInfo.currentLocation += 1;
      } else {
        this.$store.state.dataInfo.marked += 1
        this.$store.state.dataInfo.unmark -= 1
      ...
      // this.upload(uuid, filename, label_, ner, tuple)
      // debugger
      // console.log(this.allIntentions[this.$store.state.dataInfo.currentLocation].label.length > 0 && category === label_)
    },

    // classification按钮函数
    upload (uuid, filename, label, ner, tuple) {
      if (this.allIntentions[this.$store.state.dataInfo.currentLocation].sentence === '') {
        this.$message({
          message: 'data error',
          type: 'error',
          duration: 1500
      ...
      // console.log(789456)
      // console.log(label)
      const url = this.url + 'mark_multiclassification/'
      // let newLabel=''
      // label.forEach((item)=>{
      //   newLabel
      // })
      ...
      // debugger
      // params.append("label", label.replace(',',''));
